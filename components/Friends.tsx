...
import { baseGift, bigGift, dollarCoin, gift_box_1, gift_box_2, hamsterCoin } from '@/images';
...
export default function Friends() {
  ...
  const handleCopyInviteLink = useCallback(() => {
    triggerHapticFeedback(window);
    navigator.clipboard
      .writeText(process.env.NEXT_PUBLIC_BOT_USERNAME ? `https://t.me/${process.env.NEXT_PUBLIC_BOT_USERNAME}/${process.env.NEXT_PUBLIC_APP_URL_SHORT_NAME}?startapp=kentId${getUserTelegramId(userTelegramInitData) || ""}` : "https://t.me/game_news")
      .then(() => {
  ...
  const handleInviteFriend = useCallback(() => {
    ...
    const inviteLink = botUsername
      ...
      : "https://t.me/game_news";

    const shareText = `🎮 Join : Tap, Earn, and Win! 🏆\n🚀 Let's play and earn together!`;
                ...
                <div className="space-y-2">
                  <div className="flex justify-between items-center bg-[#272a2f] rounded-lg p-4">
                    <div className="flex items-center text-sm">
                      <Image src={gift_box_2} alt="Gift" width={40} height={40} />
                      <div className="flex flex-col ml-2">
                        <span >Invite a friend</span>
                        <div className="flex items-center">
                          <Image src={dollarCoin} alt="Premium Gift" width={18} height={18} />
                          <span className="ml-1 text-white"><span className="text-[#f3ba2f]">+{formatNumber(REFERRAL_BONUS_BASE)}</span><span className="font-medium"> for you and your friend</span></span>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex justify-between items-center bg-[#272a2f] rounded-lg p-4">
                    <div className="flex items-center text-sm">
                      <Image src={gift_box_1} alt="Premium Gift" width={40} height={40} />
                      <div className="flex flex-col ml-2">
                        <span >Invite a friend with Telegram Premium</span>
                        <div className="flex items-center text-sm">
                          {/* <IceCube className="w-6 h-6" /> */}
                          <Image src={dollarCoin} alt="Premium Gift" width={18} height={18} />
                          <span className="ml-1 text-white"><span className="text-[#f3ba2f]">+{formatNumber(REFERRAL_BONUS_PREMIUM)}</span><span className="font-medium"> for you and your friend</span></span>
                        </div>
                      </div>
                    </div>
                  </div>
                      ...
                      <div key={index} className="flex items-center bg-[#272a2f] rounded-lg p-4">
                        ...
                        <div className="flex items-center justify-between flex-1">
                          <div className="flex items-center mr-4">
                            <Image src={dollarCoin} className='w-4 h-4 mr-1' alt='dollar coin' />
