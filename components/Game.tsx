...
import { binanceLogo, Crypty, daily_cipher, daily_combo, daily_reward, dailyCipher, dailyCombo, dailyReward, dollarCoin, lightning, mini_game } from '@/images';
  ...
  return (
    <div className="bg-black flex justify-center min-h-screen">
      <div className="w-full bg-black text-white h-screen font-bold flex flex-col max-w-xl">
        ...
        <div className="flex-grow mt-4 bg-[#f3ba2f] rounded-t-[48px] relative top-glow z-0">
          <div className="mt-[2px] bg-[#1d2025] rounded-t-[46px] h-full overflow-y-auto no-scrollbar">
            ...
            <div className="px-1 mt-6 flex justify-between gap-2">
              <div className="bg-[#272a2f] rounded-lg py-2 w-full relative">
                <div className="dot"></div>
                <Image src={daily_reward} alt="Daily Reward" className="mx-auto w-12 h-12" />
                <p className="text-[10px] text-center text-white mt-1 whitespace-nowrap">Daily reward</p>
                <p className="text-[10px] font-medium text-center text-gray-400 mt-2">{dailyRewardTimeLeft}</p>
              </div>
              <div className="bg-[#272a2f] rounded-lg  py-2 w-full relative">
                <div className="dot"></div>
                <Image src={daily_cipher} alt="Daily Cipher" className="mx-auto w-12 h-12" />
                <p className="text-[10px] text-center text-white mt-1 whitespace-nowrap">Daily cipher</p>
                <p className="text-[10px] font-medium text-center text-gray-400 mt-2">{dailyCipherTimeLeft}</p>
              </div>
              <div className="bg-[#272a2f] rounded-lg  py-2 w-full relative">
                <div className="dot"></div>
                <Image src={daily_combo} alt="Daily Combo" className="mx-auto w-12 h-12" />
                <p className="text-[10px] text-center text-white mt-1 whitespace-nowrap">Daily combo</p>
                <p className="text-[10px] font-medium text-center text-gray-400 mt-2">{dailyComboTimeLeft}</p>
              </div>
              <div className="bg-[#272a2f] rounded-lg  py-2 w-full relative">
                <div className="dot"></div>
                <Image src={mini_game} alt="Daily Combo" className="mx-auto w-12 h-12" />
                <p className="text-[10px] text-center text-white mt-1 whitespace-nowrap">Minigames</p>
              </div>
            </div>
              ...
              <div className="px-4 mt-4 flex justify-center">
                ...
                >
                  ...
                  {/* LEVELS[gameLevelIndex].bigImage scale(1.05) translateY(10%)*/}
                    <Image
                      src={Crypty}
                      alt="Main Character"
                      fill
              ...
              <div className="flex justify-between px-4 mt-4">
                <p className="flex justify-center items-center gap-1">
                  <Image src={lightning} alt="Exchange" width={35} height={35} />
                  <span className="flex">
                    <span className="text-base font-medium">{energy}</span>
                    <span className="text-base font-medium"> / {maxEnergy}</span>
                  </span>
                  </p>
                <button onClick={() => handleViewChange("boost")} className="flex justify-center items-center gap-1"><Rocket size={40} /><span className="text-base">Boost</span></button>
              </div>
      ...
      {clicks.map((click) => (
        <div
          key={click.id}
          className="absolute text-3xl font-bold opacity-0 text-white pointer-events-none flex justify-center"
          style={{
            top: `${click.y - 42}px`,
            left: `${click.x - 28}px`,
        ...
        >
          +{pointsPerClick}
          <Image src={dollarCoin} className='w-8 h-8 mx-auto' alt='dollar coin' />
        </div>
      ))}
    </div>
  )
