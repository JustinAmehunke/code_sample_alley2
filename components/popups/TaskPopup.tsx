...
import { dollarCoin, imageMap } from '@/images';
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-end justify-center z-50">
      <div className={`bg-[#272a2f] rounded-3xl p-6 w-full max-w-xl ${isClosing ? 'animate-slide-down' : 'animate-slide-up'}`}>
        <div className="flex justify-between items-center mb-4">
          ...
          <h2 className="text-2xl text-white text-center font-bold">{localTask.title}</h2>
        ...
        {localTask.taskData.link && (
          <div className="flex justify-center mb-4">
            <button
              className="w-fit px-6 py-3 text-xl font-bold bg-[#5A60FF] text-white rounded-2xl"
              onClick={() => {
                triggerHapticFeedback(window);
                window.open(localTask.taskData.link, '_blank');
              }}
        ...
        <Image src={dollarCoin} alt="dollar coin" className="w-6 h-6 mr-1" />
        ...
        {localTask.type === 'VISIT' ? (
          ...
          <button
            className="w-full py-6 text-xl font-bold bg-[#5A60FF] text-white rounded-2xl flex items-center justify-center"
            onClick={handleCheck}
